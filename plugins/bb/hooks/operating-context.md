...
- **English prose follows one set of style rules.** The plugin-level `references/doc-style.md` states them whole, for every English sentence bb writes, in its own files and in the documents it generates. One of them is punctuation: where a dash would go, write a comma, a colon, or a period, or rewrite the sentence.
