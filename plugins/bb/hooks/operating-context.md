...
- **Spec before building anything non-trivial.** For a real feature, reach for `/bb:spec` first — develop a draft, loop through the gray areas as questions, converge on a brief, _then_ build. Skip it for tiny mechanical changes; just do those. Upstream of the idea, `/bb:discover` frames the problem and its fit.
