...
- **English prose follows one style ruler.** The plugin-level `references/doc-style.md` carries it whole, for every English sentence bb writes, in its own files and in the documents it generates. One of its rules crosses into Portuguese too, because it is punctuation and not vocabulary: where a dash would go, write a comma, a colon, or a period, or rewrite the sentence.
