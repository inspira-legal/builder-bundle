...
The on-disk contract — location, frontmatter schema, status lifecycle, and the read-only fallback to legacy `.ofc/tasks/<slug>/shape.md` briefs — is the plugin-level `references/task-state.md`; follow it. In short: new briefs always go to `.bb/`; a legacy brief found for the same idea is updated in place, never moved. If a brief already exists for a _different_ idea under the same slug, suffix it (`-2`) or ask — never silently overwrite another brief.
