...
the target inline, so those two values are materialized, each naming the discovery record
in its provenance note, and once the gate hardens them the spec's values win, per the
reversal rule in the plugin-level `references/spec-state.md`. So don't re-litigate a cut
the user made upstream, and don't ask
