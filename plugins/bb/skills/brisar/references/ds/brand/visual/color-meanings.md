...
Colors are not just hues — each Inspira color carries a specific emotional role. When choosing a color, the _intent_ matters as much as the _scale position_. Use this file to decide **which color** to use; use the `primitives.css` section of `tokens/tokens-css.md` to decide **which shade**.
