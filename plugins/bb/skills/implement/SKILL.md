...
6. **Build every unchecked task, respecting `dep:`. The build is dispatched, not run here.** One agent per task, as a dynamic workflow, run by the fixed script the plugin ships. **Invoking this skill is the request for that workflow**, and that request is the opt-in the `Workflow` tool asks for, for this build. The build lands back in this context only at the last step of the reference's fallback chain. **Read the plugin-level `references/build-tasks-workflow.md` before invoking**: it carries the `args` shape, the per-run items the skill confirms, the `Bash` call that normalizes the script into the copy `scriptPath` points at, the invoke line, the fallback chain and what the return means. Three things the reference leaves to this skill: `checks` is `<plugin-root>/scripts/resolve_checks.py`'s output passed through whole, the same call step 7 makes, so the chain is walked once per run and stage zero is what runs what it found; `tasks` carries the **unticked ones only**, ordered so `dep:` is already satisfied; and **`phases` is read off the `###` headings inside `## Tasks`**, whose shape the reference states. Those titles are what the progress card shows as its group headers, so take them from the spec and never invent one here: a name written per run is unreviewed, and it invalidates the resume cache. **With every task already ticked there's nothing to dispatch**: say so and go on to whatever the scope has next, which reads a build with nothing to do as a clean one.
