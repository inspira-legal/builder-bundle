...
description: Gather and summarize all changes on the current branch compared to main. Use when user says "compare against main", "branch context", "what changed on this branch", "summarize my branch", "diff against main", "review my changes", or "what did I do on this branch". Do NOT use to fix or green a PR (use /ofc:ship), open a PR (use /ofc:ship), or improve the diff's quality (use /ofc:improve-code) — this only summarizes, it never edits.
license: MIT
metadata:
